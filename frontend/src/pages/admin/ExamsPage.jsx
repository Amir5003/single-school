...
import { getAdminExams, createExam, deleteExam } from '../../api/exam.api';
        ...
        {form.subjects.map((s, i) => (
          <div key={i} className="flex gap-2 mb-1 items-center">
            ...
            <div className="flex items-center gap-1">
              <label className="text-xs text-gray-500 whitespace-nowrap">MM</label>
              <input
                type="number"
                min={1}
                max={1000}
                ...
                className="w-20 border border-gray-300 rounded-lg px-2 py-1.5 text-sm outline-none focus:ring-2 focus:ring-indigo-400"
                placeholder="100"
              />
            </div>
            {form.subjects.length > 1 && (
              <button type="button" onClick={() => removeSubject(i)} className="text-red-400 hover:text-red-600 text-xs px-2">✕</button>
            )}
          </div>
        ))}
