...
- ~~The legacy "marks" model (`Marks.model.js`, used by `MarksPage` for teachers) is unrelated to this feature and is left untouched.~~ **Superseded by spec 008** (`specs/008-coursework-report-cards/`): that model is now "Coursework", its `examType` field is renamed `assessmentType`, and the `midterm`/`final` values are removed — they allowed a teacher to record term-exam marks outside this feature's publish workflow. The Exam/SubjectSubmission/Result pipeline specified here is unchanged.
